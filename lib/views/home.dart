...
import 'package:courser/services/subject_service.dart';
import 'package:courser/views/subject_page.dart';
...
class _HomeState extends State<Home> {
  ...
  void displayMenu({BuildContext? context, VoidCallback? menuTapHandler, String? popupMenuValue, String? displayText}) {
    showMenu(context: context!, position: const RelativeRect.fromLTRB(50, -50, 10, 0), items: [
      PopupMenuItem(
              ...
              children: [
                Flexible(
                  child: Text(
                    ...
                    style: Get.isDarkMode ? CourserTheme.darkTextTheme.headline3 : CourserTheme.lightTextTheme.headline3,
                  ),
                ),
                Flexible(
                  child: Text(
                    auth.auth.currentUser!.email!,
                    style: Get.isDarkMode ? CourserTheme.darkTextTheme.headline5 : CourserTheme.lightTextTheme.headline5,
                  ),
                ),
              ],
            ),
  ...
  Widget displayCalendar() {
    return Card(
        color: Get.isDarkMode ? const Color.fromARGB(90, 148, 87, 142) : const Color.fromARGB(90, 247, 174, 239),
        margin: const EdgeInsets.all(4),
        child: Padding(
          padding: const EdgeInsets.all(8.0),
          child: Column(
  ...
  Widget displayEnrolledCard() {
    return Card(
      color: Get.isDarkMode ? const Color.fromARGB(90, 24, 29, 70) : const Color.fromARGB(133, 148, 148, 196),
      margin: const EdgeInsets.all(4),
      child: Column(
        children: [
          const Padding(
                    ...
                    Expanded(
                      child: Column(
                        children: [
                          Expanded(
                            flex: 5,
                            child: SizedBox(height: size.height, width: size.width, child: displayUserDetails()),
                          ),
                          Expanded(
                            flex: 5,
                            child: SizedBox(
                      ...
                      Expanded(
                        child: SizedBox(
                          ...
                          child: _loading!
                              ...
                              : ListView.builder(
                                  primary: true,
                                  itemCount: _loading! ? 0 : _allSubjects!.length,
                                  itemBuilder: (BuildContext context, int index) {
                                    ...
                                    return ListTile(
                                      ...
                                      trailing: ActionChip(
                                        label: Text(_enrolled ? 'Unenroll' : 'Enroll'),
                                        onPressed: () {
                                          chipTapHandler(
                                            index: index,
                                      ...
                                      onTap: () {
                                        Get.to(() => SubjectPage(allSubjects: subject));
                                      },
                                    );
                                  },
                                ),
